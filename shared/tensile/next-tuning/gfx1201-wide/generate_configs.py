...
Generated files (84 total):
  WMMA V2 path — gfx1201-native instructions:
    ...
    bf16gemm_wmma_native_{nn,nt,tn,tt}.yaml — bf16 native (V_WMMA_BF16_16X16X16_BF16, D=bf16 acc)
  ...
  VALU path (fallback + non-WMMA types):
    ...
    bf16gemm_native_{nn,nt,tn,tt}.yaml    — bf16 native (non-HPA) VALU
...
WMMA instruction coverage (gfx1201 / Navi48):
  covered:   V_WMMA_F32_16X16X16_F16/BF16 (both D=f16/bf16 and D=f32 outputs)
             V_WMMA_F16_16X16X16_F16, V_WMMA_BF16_16X16X16_BF16 (native accum)
...
# M: decode (16) → cont-batching (64,256) → prefill (1024,4096)
...
def problem_type_wmma_bf16gemm_native(ta, tb, desc):
    return f"""\
    - # {desc} — bf16 native WMMA (V_WMMA_BF16_16X16X16_BF16 — bf16 accumulate)
      OperationType: GEMM
      DataType: B
      ComputeDataType: B
      DestDataType: B
...
def problem_type_valu_bf16gemm_native(ta, tb, desc):
    return f"""\
    - # {desc} — bf16 native VALU (bf16 accumulate, no HPA — BBB)
      OperationType: GEMM
      DataType: B
      ComputeDataType: B
      DestDataType: B
...
def configs():
    ...
    for tsuffix, (ta, tb, tdesc) in TRANSPOSES.items():
        ...
        # ── WMMA bf16 native (V_WMMA_BF16_16X16X16_BF16) ─────────────────
        yield (
            f"bf16gemm_wmma_native_{tsuffix}.yaml",
            build_yaml(
                problem_type_wmma_bf16gemm_native(ta, tb, tdesc),
                COMMON_PARAMS_WMMA,
                wmma_fork_params(),
                sizes,
                f"bf16gemm_wmma_native_{tsuffix}",
            )
        )
        ...
        # ── VALU bf16 native (non-HPA — BBB) ────────────────────────
        yield (
            f"bf16gemm_native_{tsuffix}.yaml",
            build_yaml(
                problem_type_valu_bf16gemm_native(ta, tb, tdesc),
                COMMON_PARAMS_VALU,
                valu_fork_params_native(),
                sizes,
                f"bf16gemm_native_{tsuffix}",
            )
        )
