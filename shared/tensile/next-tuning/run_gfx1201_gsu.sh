#!/usr/bin/env bash
# =============================================================================
# run_gfx1201_gsu.sh — gfx1201 (Navi48) GSU / WorkGroupMapping sweep
#
# Runs every YAML in gfx1201-gsu/ sequentially through Tensile.
# Each config is isolated: a failure is logged to the JSONL results file
# and the script continues with the next config.
#
# Usage:
#   ./run_gfx1201_gsu.sh                       # run all 24 configs
#   ./run_gfx1201_gsu.sh --rerun-failed         # only re-run configs marked FAILED
#   ./run_gfx1201_gsu.sh --filter "wmma"        # only run configs whose name contains "wmma"
#   ./run_gfx1201_gsu.sh --dry-run              # list what would run, don't execute
#
# Results:
#   $RESULTS_JSONL  — JSONL file, one record per run (append-safe, timestamped)
#   $GSU_DIR/*.log  — per-config Tensile stdout/stderr logs
#
# =============================================================================
set -uo pipefail
...
GSU_DIR="${SCRIPT_DIR}/gfx1201-gsu"
RESULTS_JSONL="${SCRIPT_DIR}/gfx1201_gsu_results.jsonl"
...
OUTPUT_BASE="${OUTPUT_BASE:-/home/iamthecage/out_gsu}"
...
if [[ ! -d "${GSU_DIR}" ]]; then
    echo "[ERROR] GSU config dir not found: ${GSU_DIR}"
    exit 1
fi

# ── Build file list ───────────────────────────────────────────────────────────
mapfile -t ALL_YAMLS < <(find "${GSU_DIR}" -maxdepth 1 -name '*.yaml' | sort)
