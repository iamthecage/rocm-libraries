#!/usr/bin/env bash
# =============================================================================
# run_gfx1201_cumode.sh — gfx1201 (Navi48) CU-mode kernel sweep
#
# Runs every YAML in gfx1201-cumode/ sequentially through Tensile.
# Each config is isolated: a failure is logged to the JSONL results file
# and the script continues with the next config.
#
# Usage:
#   ./run_gfx1201_cumode.sh                       # run all configs
#   ./run_gfx1201_cumode.sh --rerun-failed         # only re-run configs marked FAILED
#   ./run_gfx1201_cumode.sh --filter "wmma"        # only run configs whose name contains "wmma"
#   ./run_gfx1201_cumode.sh --dry-run              # list what would run, don't execute
#
# Results:
#   $RESULTS_JSONL  — JSONL file, one record per run (append-safe, timestamped)
#   $CUMODE_DIR/*.log  — per-config Tensile stdout/stderr logs
#
# =============================================================================
set -uo pipefail
...
CUMODE_DIR="${SCRIPT_DIR}/gfx1201-cumode"
RESULTS_JSONL="${SCRIPT_DIR}/gfx1201_cumode_results.jsonl"
...
OUTPUT_BASE="${OUTPUT_BASE:-/home/iamthecage/out_cumode}"
...
if [[ ! -d "${CUMODE_DIR}" ]]; then
    echo "[ERROR] CU-mode config dir not found: ${CUMODE_DIR}"
    exit 1
fi

# ── Build file list ───────────────────────────────────────────────────────────
mapfile -t ALL_YAMLS < <(find "${CUMODE_DIR}" -maxdepth 1 -name '*.yaml' | sort)
