...
reference (fp32 GPU), catching silent-corruption bugs in new kernels.
...
Additional options:
  ...
  --no-tune                         Disable PyTorch TunableOp exhaustive search (removes delays)
...
def verify_gemm(M: int, N: int, K: int, dtype, dtype_name: str,
    ...
    """
    Verify a single GEMM by comparing GPU result against an fp32 GPU reference.
    ...
    # Generate directly on GPU using fast normal_ distribution
    A_gpu = torch.empty(M, K, dtype=dtype, device=device).normal_()
    B_gpu = torch.empty(K, N, dtype=dtype, device=device).normal_()

    # Reference: upcast to fp32 (NOT fp64) ON THE GPU for the ground-truth.
    # Consumer/workstation AMD GPUs (gfx1201) severely throttle fp64 math,
    # which causes massive kernel stalls. FP32 is fully accelerated.
    ref_gpu = torch.mm(A_gpu.float(), B_gpu.float())

    # GPU result in native dtype
    result_gpu = torch.mm(A_gpu, B_gpu)

    # Compute error entirely on the GPU. Moving multi-million element tensors 
    # to the CPU and doing element-wise math pegs the CPU to 100%.
    result_gpu_f32 = result_gpu.float()
    
    abs_err = (result_gpu_f32 - ref_gpu).abs()
    # For relative error, normalize by max(|ref|, 1) to avoid div-by-zero on small values
    rel_err = abs_err / (ref_gpu.abs().clamp(min=1.0))

    # Extract only the final scalar values to the CPU
    max_abs = abs_err.max().item()
    max_rel = rel_err.max().item()
    mean_abs = abs_err.mean().item()

    # Count mismatched elements (outside tolerance) on the GPU
    mismatched = ((abs_err > atol) & (rel_err > rtol)).sum().item()
    total = M * N

    passed = mismatched == 0

    del A_gpu, B_gpu, result_gpu, ref_gpu, result_gpu_f32, abs_err, rel_err
...
def benchmark_gemm(M: int, N: int, K: int, dtype, warmup: int, iters: int,
    ...
    # Use empty().normal_() to bypass slow CPU-fallback RNG bottlenecks seen in older ROCm PyTorch
    A = torch.empty(M, K, dtype=dtype, device=device).normal_()
    B = torch.empty(K, N, dtype=dtype, device=device).normal_()

    # Warmup
    # NOTE: If TunableOp is enabled, this is where PyTorch pauses to search the entire kernel space.
    ...
    # Cap repeats to 1000 to prevent CPU queue starvation/hangs on ultra-fast shapes
    repeats = min(repeats, 1000)
...
def benchmark_layer(model: ModelConfig, M: int, dtype, dtype_name: str,
    ...
    for op_name, m, n, k, trans in gemm_shapes:
        # Give dynamic visual feedback
        print(f"    -> Tuning & timing {op_name:<15} {m:>5}×{n:>5}×{k:>5}...", end="", flush=True)
        ...
        reps = result.get('repeats', 1)
        rep_str = f" r={reps}" if reps > 1 else ""
        v_sym = ""
        if do_verify:
            v_sym = " ✓" if result.get("verify", {}).get("pass") else f" ✗ err={result.get('verify', {}).get('max_abs_err','?')}"
            
        print(f" {result['median_ms']:.4f}ms  {result['tflops']:>7.2f} TF{rep_str}{v_sym}")
    ...
    layer_repeats = min(layer_repeats, 100) # Cap to prevent hang
...
def setup_tunableop(output_dir: str, tag: str, enable_tuning: bool) -> str:
    """
    ...
    # Stop ROCm from active spin-polling (which pegs the CPU core at 100% while waiting for GPU logic like Tuning)
    #os.environ["HSA_YIELD_STRATEGY"] = "2"
    ...
    #if enable_tuning:
        # Enable tuning so it actually exhaustively tries hundreds of kernels and records the winner.
        # WARNING: This causes a multi-second delay and 100% CPU lock during the search for EVERY new matrix shape.
        #os.environ["PYTORCH_TUNABLEOP_TUNING"] = "1"
    #else:
        # Disable exhaustive search. PyTorch will instantly pick the default kernel using standard rocBLAS heuristics.
...
def main():
    ...
    parser.add_argument("--no-tune", action="store_true",
                        help="Disable PyTorch TunableOp exhaustive search (removes multi-second delays between shapes)")
    ...
    tunableop_file = setup_tunableop(args.output, args.tag, not args.no_tune)
    ...
    print(f"TunableOp search: {'ENABLED (Expect multi-second tuning delays per new shape)' if not args.no_tune else 'DISABLED (Fast mode)'}")
    ...
    if args.smoke_test:
        ...
        print(f"  Comparing GPU rocBLAS results against fp32 GPU reference")
        print(f"{'━' * 80}")
    ...
    for model_name in model_names:
        ...
        for scenario_name in scenario_names:
            ...
            print(f"    [Layer Composite] {layer_result['layer_median_ms']:.3f}ms, "
